...
import { db, getStorage } from "../firebase-config";
import {
  collection,
  addDoc,
  updateDoc,
...
const Register = () => {
  ...
  const [resumeMember, setResumeMember] = useState(null);
  ...
  // const resumeRegex = /^(https?:\/\/)?(www\.)?drive\.google.com\/in\/[a-zA-Z0-9_-]+$/;
  ...
  const onSubmit = async (e) => {
    ...
    if (resumeMember) {
      ...
      try {
        ...
        // Now you have the resume download URL, you can store it in your Firestore document or do other operations
      } catch (error) {
        // Handle error
        console.error("Error uploading resume:", error);
        setAlertMessage("Error uploading resume. Please try again later.");
        setAlertType("error");
        setShowAlert(true);
        return;
      }
    ...
    if (imageFile) {
      ...
      try {
        ...
        // Now you have the image download URL, you can store it in your Firestore document or do other operations
      } catch (error) {
        // Handle error
        console.error("Error uploading image:", error);
        setAlertMessage("Error uploading image. Please try again later.");
        setAlertType("error");
        setShowAlert(true);
    ...
    // If everything is successful, proceed with other form submission logic
    // setLoading(true);
    // Continue with Firestore operations, etc.
          ...
          if (data.teamName === teamName) {
            ...
            if (
              ...
              updateDoc(docRef.ref, {
                ...
                resumeMember3: resumeMember,
                imageMember3: imageFile,
            ...
            if (
              ...
              updateDoc(docRef.ref, {
                ...
                resumeMember2: resumeMember,
                imageMember2: imageFile,
          ...
          <form className="grid sm:grid-cols-1 md:grid-cols-1 lg:grid-cols-2 gap-4">
            <fieldset className="border border-orange-600 p-6 rounded-2xl m-3 bg-blur">
              ...
              <input
                ...
                type="file"
                accept=".pdf"
                onChange={(e) => setResumeMember(e.target.files[0])}
                required
              />
            ...
            <aside className="border border-orange-600 rounded-2xl p-3 mt-[26px] mx-2 mb-3 flex-1">
              <div className="bg-blur rounded-2xl h-full p-4">
                ...
                <ul className="list-disc mt-4 list-inside text-lg">
                  <li>
                    Ensure that the your email and the one you used for logging
                    in are both same.
                  </li>
                  <li>
                    Use existing team name to register your self as a part of
                    existing team.
                  </li>
                  <li>
                    Enter new team name to create a new team and share that team
                    name with your members so that they can become part of your
                    team.
                  </li>
                  <li>
                  ...
                  <li>
                    Provide Resume link Make sure that your photo must be there
                    in your resume
                  </li>
                </ul>
                <h2 className="font-bold text-1xl text-center mt-10">
                  {" "}
                  Note: Please read instructions carefullly!!!
                </h2>
              </div>
            </aside>
            ...
            <div className="flex justify-center text-center md:text-left font-bold">
              {user ? (
                <button
                  className={`mt-3 ${
                    isMobile ? "mx-2" : "mx-3"
                  } text-lg bg-orange-600 hover:bg-orange-800 px-4 py-2 text-white uppercase rounded-xl`}
                  type="submit"
                  style={{
                    width: "100%",
                    display: "flex",
                ...
                <Link
                  ...
                  className={`mt-3 ${
                    isMobile ? "mx-2" : "mx-3"
                  } text-lg bg-white px-4 py-2 text-orange-600 uppercase rounded-xl`}
                  style={{
                    width: "100%",
                    display: "flex",
                    justifyContent: "center",
